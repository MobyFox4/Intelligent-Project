...
if page == "Machine Learning":
    ...
    st.header("🤖 ฝึกโมเดล")
    st.text("")
    st.markdown("""โมเดลตัวแรกจะเลือกใช้ <span style="color:green; font-weight:bold;">Random Forest</span> ซึ่งเป็นอัลกอริธึมของ Machine Learning ที่ใช้สำหรับ Classification ซึ่งวิธีการของ Random Forest จะสร้าง Dicision Trees ออกมาหลายๆต้นออกมา แล้วจะนำผลลัพธ์ที่ได้จาก Dicision Trees ทั้งหมดมาเช็คดูว่าผลลัพธ์ที่ออกมาว่าส่วนใหญ่เป็นอะไร แล้วจะให้ผลลัพธ์ส่วนใหญ่นั้นเป็นคำตอบออกมา หลังจากที่ได้โมเดลออกมาแล้วก็ลองทำการทดสอบโมเดลเพื่อดูค่า Confusion Matrix เพื่อแสดงจำนวน ค่าทำนายที่ถูกต้องและผิดพลาด | Accuracy ใช้วัดค่าความแม่นยำของโมเดล | Precision ใช้วัดความแม่นยำของการทำนายผล | Recall ใช้วัดค่าความถูกต้อง | F1 Score ใช้วัดค่าความสมดุลระหว่าง Precision และ Recall""", unsafe_allow_html=True)
    st.code(MLcode8, language="python")
    st.image("Streamlit/image/ML/ML7.png")
    
    st.text("")
    st.markdown("""โมเดลที่สองจะเลือกใช้ <span style="color:green; font-weight:bold;">Logistic Regression</span> เป็นอัลกอริธึมของ Machine Learning ที่ใช้สำหรับ Classification เหมือนกัน วิธีการของ Logistic Regression จะใช้สมการเส้นตรง แล้วนำค่าที่ได้ไปผ่านฟังก์ชัน Sigmoid เพื่อแปลงผลลัพธ์เป็นค่าความน่าจะเป็น แล้วตัดสินว่าอยู่ในกลุ่มไหน หลังจากที่ได้โมเดลออกมาแล้วก็ลองทำการทดสอบโมเดลเพื่อดูค่า Confusion Matrix, Accuracy, Precision, Recall, และ F1 Score เหมือนกับโมเดลแรก""", unsafe_allow_html=True)
